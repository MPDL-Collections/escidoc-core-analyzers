...
public class TestStandardAnalyzer extends TestBase {

	private StandardAnalyzer analyzer;

	@Test
	public void testLuceneVersion_3_3() throws Exception {
		
		analyzer = new StandardAnalyzer(Version.LUCENE_33);
		tokenizeWithTokenizer(analyzer);
	...
	private void tokenizeWithTokenizer(Analyzer analyzer) throws IOException {
		TokenStream result;
		...
		assertTrue(tokens.get(1).equals("cfg"));
		tokens.clear();
		...
		assertTrue(tokens.get(0).equals("abc.def"));
		tokens.clear();

		result = analyzer.tokenStream("field", new StringReader(
				"2011-01-28T15:14:30.925Z"));
		tokens = doTokenizing(result);
		assertTrue(tokens.size() == 5);
		assertTrue(tokens.get(0).equals("2011"));
		assertTrue(tokens.get(1).equals("01"));
		assertTrue(tokens.get(2).equals("28t15"));
		assertTrue(tokens.get(3).equals("14"));
		assertTrue(tokens.get(4).equals("30.925z"));
		tokens.clear();
		...
		assertTrue(tokens.get(0).equals("hdl"));
		assertTrue(tokens.get(1).equals("11858"));
		assertTrue(tokens.get(2).equals("00"));
		assertTrue(tokens.get(3).equals("001z"));
		assertTrue(tokens.get(4).equals("0000"));
		assertTrue(tokens.get(5).equals("0023"));
		assertTrue(tokens.get(6).equals("453d"));
		assertTrue(tokens.get(7).equals("4"));

		result = analyzer.tokenStream("field", new StringReader(
				"http://pubman.mpdl.mpg.de/pubman"));
		tokens = doTokenizing(result);
		assertTrue(tokens.size() == 3);
		assertTrue(tokens.get(0).equals("http"));
		assertTrue(tokens.get(1).equals("pubman.mpdl.mpg.de"));
		assertTrue(tokens.get(2).equals("pubman"));
		...
		result = analyzer.tokenStream("field", new StringReader(
				"quasar SDSS J172206.03+565451.6"));
		tokens = doTokenizing(result);
		assertTrue(tokens.size() == 5);
		...
		result = analyzer.tokenStream("field",
				new StringReader("Pb-207/Pb-204"));
		tokens = doTokenizing(result);
		assertTrue(tokens.size() == 1);
		...
		result = analyzer
				.tokenStream("field", new StringReader("Xaver Müller"));
		tokens = doTokenizing(result);
		assertTrue(tokens.size() == 2);
		...
		result = analyzer.tokenStream("field",
				new StringReader("God and Right"));
		tokens = doTokenizing(result);
		assertTrue(tokens.size() == 2);
