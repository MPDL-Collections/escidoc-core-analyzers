...
public class EscidocJapaneseAnalyzer extends Analyzer {
    ...
    public TokenStream tokenStream(final String fieldName, final Reader reader) {
        ...
        TokenStream whitespaceTokens = new WhitespaceTokenizer(reader);
        Reader reader1 = null;
        try {
            StringBuffer tokenBuffer = new StringBuffer("");
            TermAttribute termAtt = whitespaceTokens.addAttribute(TermAttribute.class);
            whitespaceTokens.reset();
            while (whitespaceTokens.incrementToken()) {
                if (tokenBuffer.length() > 0) {
                    tokenBuffer.append(" ");
                }
                tokenBuffer.append(termAtt.term());
            }
